defmodule SmokeShopWeb.ProductController do
  ...
  def show(conn, %{"id" => id}) do
    case get_session(conn, :user_id) do
      ...
      val when is_integer(val) ->
        ...
        conn
        |> simple_cart(product.id)
        |> render("show.html", product: product, user: user)
    end
  end

  defp simple_cart(conn, product_id) do
    case get_session(conn, :cart_ids) do
      nil -> put_session(conn, :cart_ids, [product_id])
      list -> put_session(conn, :cart_ids, list ++ [product_id])
    end
  end
